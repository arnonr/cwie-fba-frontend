<template>
  ...
  <b-card no-body>
    <b-overlay :show="isOverLay" opacity="0.3" spinner-variant="primary">
      ...
      <document-type-view v-model="isViewModal" :item="item" />
    </b-overlay>
  </b-card>
</template>
...
import DocumentTypeView from "./DocumentTypeView.vue";

import documentTypeStoreModule from "./documentTypeStoreModule";
...
export default {
  ...
  setup() {
    const DOCUMENT_TYPE_APP_STORE_MODULE_NAME = "document-type";

    // Register module
    if (!store.hasModule(DOCUMENT_TYPE_APP_STORE_MODULE_NAME))
      store.registerModule(
        DOCUMENT_TYPE_APP_STORE_MODULE_NAME,
        documentTypeStoreModule
      );

    // UnRegister on leave
    onUnmounted(() => {
      if (store.hasModule(DOCUMENT_TYPE_APP_STORE_MODULE_NAME))
        store.unregisterModule(DOCUMENT_TYPE_APP_STORE_MODULE_NAME);
    });
    ...
    const handleAddFormClick = () => {
      item.value = JSON.parse(JSON.stringify(blankDocumentType));
      isModal.value = true;
    };
